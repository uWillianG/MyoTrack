...
- [x] **Fase 3 — Análise de vídeo de exercício**: upload direto no MinIO via URL pré-assinada (até 100 MB / 60 s), serviço `vision` (Python + FastAPI + MediaPipe Pose) processa a ~12 fps, heurísticas para agachamento, levantamento terra e desenvolvimento (câmera lateral), contagem de repetições, score conservador ("não avaliável" em vez de feedback errado), vídeo com overlay do esqueleto (H.264) e erros com timestamps clicáveis no player. Limite diário configurável por plano (`Limits:Free:MaxVideoAnalysesPerDay`, padrão 5; Pro 20).
