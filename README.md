...
- [x] **Fase 3 — Análise de vídeo de exercício**: upload direto no MinIO via URL pré-assinada (até 100 MB / 60 s), serviço `vision` (Python + FastAPI + MediaPipe Pose) processa a ~12 fps, heurísticas declarativas para 19 exercícios (agachamento, afundo, terra, terra romeno, hip thrust, supino, flexão, mergulho em paralelas, desenvolvimento, remadas — curvada, baixa e serrote —, puxada alta, roscas — direta, martelo e Scott —, tríceps na polia, barra fixa e elevação lateral; câmera lateral), contagem de repetições, feedback com pontos corretos e pontos de atenção da execução, score conservador ("não avaliável" em vez de feedback errado), vídeo com overlay do esqueleto (H.264) e erros com timestamps clicáveis no player. Limite diário configurável por plano (`Limits:Free:MaxVideoAnalysesPerDay`, padrão 5; Pro 20).
